...
const SearchBar = () => {
  ...
  const [bertAccuracy, setBertAccuracy] = useState(null); // sửa tên hàm
  ...
  const handleSearch = async () => {
    ...
    setBertResults([]);
    setBertAccuracy(null); // sửa tên hàm
    setTfidfBertResults([]);
    ...
    try {
      const [
        ...
        axios.post(`${API_URL}/get-tfidf-bert-result`, { sentence: query, alpha: 0.8 }), // truyền alpha nếu cần
      ]);
      ...
      if (bertRes.data.output?.similarities) {
        ...
        setBertAccuracy(bertRes.data.output.accuracy ?? null);
      }

      // TF-IDF+BERT: chỉ lấy kết quả từ API, không tự xử lý
      ...
      if (
        ...
        !tfidfBertRes.data.output?.similarities?.length
      ) {
  ...
  return (
    <div className="w-full mx-auto p-4 bg-white rounded-2xl shadow-md mt-10">
      ...
      <div className="mt-6 flex flex-col md:flex-row gap-6">
        ...
        {renderResults(tfidfBertResults, 'TF-IDF + BERT', tfidfBertAccuracy)}
      </div>
    </div>
  );
};
